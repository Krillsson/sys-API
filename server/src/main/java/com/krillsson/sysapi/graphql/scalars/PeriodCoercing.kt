...
class PeriodCoercing : Coercing<Period?, String> {
    override fun serialize(input: Any): String {
        ...
        period = if (input is String) {
            Optional.ofNullable(parsePeriod(input.toString(), Function { message: String? -> CoercingSerializeException(message) }))
        } else {
    ...
    override fun parseLiteral(input: Any): Period? {
        ...
        if (input is IntValue) {
            return parsePeriod(input.value.intValueExact(), Function { message: String? -> CoercingParseLiteralException(message) })
        }
