...
class InstantCoercing : Coercing<Instant?, String> {
    override fun serialize(input: Any): String {
        ...
        instant = if (input is String) {
            Optional.ofNullable(parseInstant(input.toString(), Function { message: String? -> CoercingSerializeException(message) }))
        } else {
    ...
    override fun parseLiteral(input: Any): Instant? {
        ...
        if (input is IntValue) {
            return parseInstant(input.value.longValueExact(), Function { message: String? -> CoercingParseLiteralException(message) })
        }
