        ...
        when (state) {
            State.INSIDE -> {
                if (outsideThreshold) { //Inside -> Outside before inertia
                    ...
                    LOGGER.trace("{} went outside threshold of {} with {} at {}", config.monitoredItemId, config.threshold, value, now)
                } else {
                    LOGGER.trace("{} is still inside threshold: {} with {}", config.monitoredItemId, config.threshold, value)
                }
            }
            State.OUTSIDE_BEFORE_INERTIA -> {
                if (outsideThreshold) {
                    if (pastInertia) { //Outside before inertia -> outside
                        LOGGER.debug("{} have now been outside threshold of {} for more than {}, triggering event...", config.monitoredItemId, config.threshold, config.inertia)
                        state = State.OUTSIDE
                        stateChangedAt = null
                        ...
                        LOGGER.trace("{} is still outside threshold of {} but inside grace period of {}", config.monitoredItemId, config.threshold, config.inertia)
                    }
                } else { //Outside before inertia -> inside
                    LOGGER.trace("{} went back inside threshold of {} inside grace period of {}", config.monitoredItemId, config.threshold, config.inertia)
                    stateChangedAt = null
                    state = State.INSIDE
                }
            }
            State.OUTSIDE -> {
                if (outsideThreshold) { //Outside -> outside
                    LOGGER.trace("{} is still outside threshold of {} at {}", config.monitoredItemId, config.threshold, value)
            ...
            State.INSIDE_BEFORE_INERTIA -> {
                if (!outsideThreshold) {
                    if (pastInertia) { //Inside before inertia -> inside
                        LOGGER.debug("{} have now been inside threshold of {} for more than {}, triggering event...", config.monitoredItemId, config.threshold, config.inertia)
                        state = State.INSIDE
                        stateChangedAt = null
                        event = MonitorEvent(
                            eventId!!, monitor.id, now,
                        ...
                        LOGGER.trace("{} is still inside threshold of {} with {} but inside grace period of {}", config.monitoredItemId, config.threshold, value, config.inertia)
                    }
                } else { //Inside before inertia -> outside
                    LOGGER.trace("{} went back outside threshold of {} with {} inside grace period of {}", config.monitoredItemId, config.threshold, value, config.inertia)
                    stateChangedAt = null
                    state = State.OUTSIDE
                }
            }
