...
public class MetricsHistoryManager extends HistoryManager {
    ...
    public List<HistoryEntry<CpuLoad>> cpuLoadHistory() {
        return systemLoadHistory().stream()
                .map(e -> new HistoryEntry<CpuLoad>(e.date, e.value.getCpuLoad()))
                .collect(Collectors.toList());
    }

    public List<HistoryEntry<SystemLoad>> systemLoadHistory() {
        return getHistory();
    }

    public List<HistoryEntry<List<DriveLoad>>> driveLoadHistory() {
        return systemLoadHistory().stream()
                .map(e -> new HistoryEntry<List<DriveLoad>>(e.date, e.value.getDriveLoads()))
                .collect(Collectors.toList());
    }

    public List<HistoryEntry<List<GpuLoad>>> gpuLoadHistory() {
        return systemLoadHistory().stream()
