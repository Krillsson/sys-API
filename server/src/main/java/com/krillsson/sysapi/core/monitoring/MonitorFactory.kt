...
object MonitorFactory {
    ...
    fun createMonitor(type: Monitor.Type, id: UUID, config: MonitorConfig<MonitoredValue>): Monitor<MonitoredValue> {
        return when (type) {
            ...
            Monitor.Type.CONTAINER_RUNNING -> DockerContainerRunningMonitor(id, config as MonitorConfig<MonitoredValue.ConditionalValue>)
            ...
            Monitor.Type.PROCESS_EXISTS -> ProcessExistsMonitor(id, config as MonitorConfig<MonitoredValue.ConditionalValue>)
            Monitor.Type.CONNECTIVITY -> ConnectivityMonitor(id, config as MonitorConfig<MonitoredValue.ConditionalValue>)
            Monitor.Type.EXTERNAL_IP_CHANGED -> ExternalIpChangedMonitor(id, config as MonitorConfig<MonitoredValue.ConditionalValue>)
